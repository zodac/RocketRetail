 ...
 * <code>StockControlUtilities</code>
 */
public class StockControlUtilities {
	
	static ArrayList<OrderedItem> orderToSupplier;
	/**
	 * calculates the percentage of stock level for a product given the 
	 * current level and maximum allowable level
	...
	/**
	 * Replenishes stock levels based on the percentage of stock level they are to be increased to.
	 * If a product is below 25% and the user selects 25% that product will be replenished to 25% 
	 * @param percentage (double) current percentage of stock to order
	 * @param productsToReplenish (ArrayList) the list of products that need to be replenished
	 ...
	 * @param percentageThreshold (double)
	 * @param p (Product)
	...
	/**
	 * decides the quantity of a product to be ordered based on the current % stock level
	 * and the desired % stock level.
	 * @param percentageThreshold (double)
	 * @param p (Product)
	 * */
	...
	private static void createAndCompleteSupplierOrder(Supplier s) {
		...
		Order order = new Order(ShopDriver.orders.size(), ShopDriver.currentStaff.getStaffID(), s.getSupplierID(), new SimpleDateFormat("dd/MM/yyyy").format(new Date()), orderToSupplier, true);
		ShopDriver.orders.add(order);
	 ...
	 * @param orderToSupplier (ArrayList)
	 */
