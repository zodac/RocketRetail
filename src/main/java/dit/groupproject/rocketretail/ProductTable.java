...
public class ProductTable {
        ...
        for (int i = 0; i < ShopDriver.products.size() + 1; i++) {
            if (i < ShopDriver.products.size())
                productMemberArrayEdit[i + 1] = "ID: "
                        + doubleFormatter.format(ShopDriver.products.get(i).getProductID()) + " ("
                        + ShopDriver.products.get(i).getProductDesc() + ")";
        }
        ...
        save.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (valid) {
                    ...
                    ShopDriver.products.add(new Product(Integer.parseInt(prodIDField.getText()), prodDescField
                            .getText(), Integer.parseInt(stockLevelField.getText()), Integer.parseInt(maxLevelField
        ...
        if (ShopDriver.showDialog("Please confirm", myPanel) == JOptionPane.OK_OPTION) {
            ...
            for (Product s : ShopDriver.products) {
                if (productID == s.getProductID())
                    i = ShopDriver.products.indexOf(s); // Object can't be
                                                        // removed while being
                                                        // accessed - save its
                                                        // index location for
                                                        // now
    ...
    public static int firstAvailableID() {
        ...
        if (found)
            ...
            return ShopDriver.products.size() + ShopDriver.productIDStart;
    }
