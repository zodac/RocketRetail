...
public class ProfitLoss {
	...
	//Methods
	/**
	 * Cycles through {@link ShopDriver#orders} and adds the total values for sales, purchases and gross profit
	 * to a String.
	 * 
	 * @return			the String with the total values of sales, purchases and gross profit of the store
	 * 
	...
	public static String createTotals(){
		...
		String result = "Totals\n=====\nSales:\t\t€" + doubleFormatter.format(profit) +
						"\nPurchases:\t€" + doubleFormatter.format(loss) +
						"\n============================" + 
						profitOutput;
	...
	/**
	 * Cycles through the two 2D arrays {@link DateSort#custDates} and {@link DateSort#suppDates}, and appends
	 * the monthly totals to a StringBuilder. 
	 * 
	 * @return			a StringBuilder with the breakdown of sales and purchases by year and month
	 * 
		...
		for(int i = 0; i < ShopDriver.yearCurrent-ShopDriver.yearStart+1; i++){
			...
			for(int j = 0; j < 12; j++){
				...
				if(doubleFormatter.format(purchase).length() == 6)
					budget.append("\t"+ months[j] + "\t€" + doubleFormatter.format(purchase)+ "\t\t€"+doubleFormatter.format(sale) + "\n");
				
				else if(doubleFormatter.format(purchase).length() >= 8)
					budget.append("\t"+ months[j] + "\t€" + doubleFormatter.format(purchase)+ "\t€"+doubleFormatter.format(sale) + "\n");
				
				else budget.append("\t"+ months[j] + "\t€" + doubleFormatter.format(purchase)+ "\t\t€"+doubleFormatter.format(sale) + "\n");
			}
			year++;
		}
		return budget;
