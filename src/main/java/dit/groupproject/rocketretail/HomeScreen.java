...
public class HomeScreen {
	 ...
	 * a pie chart with the staff
	 * sales breakdown. Showing the
	 * current staff member's sales
	 * versus those of the rest of 
	...
	public static void setScreen(){
		...
		String output =  "\t"+ShopDriver.currentStaff.getStaffName() +
						"\n" + "\t"+  level + " at \"Rocket Retail Inc\" since " + ShopDriver.currentStaff.getDateAdded() 
						+ "\n" + "\t"+ "Phone Number is " + ShopDriver.currentStaff.getPhoneNumber() + "\n" + "\t"+ "Annual wage is €" +  doubleFormatter.format(ShopDriver.currentStaff.getWage()) ;
		...
		for(Order o : ShopDriver.orders){
			if(!o.isSupplier()){
				...
				for(int i = 0; i < o.getOrderedItems().size(); i++){
					orderTotal += o.getOrderedItems().get(i).getQuantity()*o.getOrderedItems().get(i).getProduct().getSalePrice();
				}
				
				if(o.getStaffID() == ShopDriver.currentStaff.getStaffID())
					staffTotal += orderTotal;
				else otherTotal += orderTotal;
		...
		ChartPanel piePanel = Graphs.createPieChart("Staff sales breakdown", ShopDriver.currentStaff.getStaffName(), salesBreakdown);
