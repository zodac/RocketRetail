...
public class Predictions {
	 ...
	 * @param yearStart An integer used to determine the start year to work from
	 * @param yearEnd An integer used to determine the year to work until
	 */
	...
	/**
	 * A method used to print the order dates to console. -Divided into customer orders and supplier orders.
	 * @param yearStart An integer used to determine the start year to work from
	 * @param yearEnd An integer used to determine the year to work until
	 */
	...
	/**
	 * A method used to fill the Customer 2D array and Supplier 2D arrays with their relevant totals.
	 * @param startYear An integer to define the start year of the method
	 * @param endYear An integer to define the end year of the method
	 */
	...
	/**
	 * Creates a 2D double array for 5 years based on the start date passed into it. Calls the createArrayOneYear 
	 * method for each of the 5 years and appends the data into the one array.
	 * @param selectedYear An integer determining the end year the data is required for
	...
	/**
	 * Creates a 2D double array for 1 year based on the start date passed into it. 
	 * (Forms an array of data for the prediction model to work on. Creates an array of customer totals 
	 * supplier totals and totals by month for one year.)
	 * @param selectedYear An integer determining the year the data is required for.
	...
	/**
	 * A method used to determine the prediction data. Bases prediction on previous 5 years data.
	 * Averages all Januarys, Februarys, Marches, Aprils, etc. To develop a prediction for the first 6 months of next year.
	 * @param dataArray The array of information to base the prediction on.
	...
	public static double[][] sixMthPrediction(double[][] dataArray){
		...
		if(total!=0){
			for(int i=0; i < 6 ; i++){
				predictionArray[i][0] = (dataArray[i][0] + dataArray[i+12][0] + dataArray[i+24][0] + dataArray[i+36][0] + dataArray[i+48][0])/5;
				predictionArray[i][1] = (dataArray[i][1] + dataArray[i+12][1] + dataArray[i+24][0] + dataArray[i+36][0] + dataArray[i+48][0])/5;
				predictionArray[i][2] = (dataArray[i][2] + dataArray[i+12][2] + dataArray[i+24][0] + dataArray[i+36][0] + dataArray[i+48][0])/5;
			}
		}
		else{
			for(k=0; k<6; k++){
	...
	/**
	 * A method used to determine the prediction data. Bases prediction on previous 5 years data.
	 * Averages all Januarys, Februarys, Marches, Aprils, etc. To develop a prediction for the next 2 years.
	 * @param dataArray The array of information to base the prediction on.
	 * @return The 2D double data array of prediction information to model.
	 */
	public static double[][] twoYrPrediction(double[][] dataArray){
		...
		for(int i=0; i < 12 ; i++){
			predictionArray[i][0] = (dataArray[i][0] + dataArray[i+12][0] + dataArray[i+24][0] + dataArray[i+36][0] + dataArray[i+48][0])/5;
			predictionArray[i][1] = (dataArray[i][1] + dataArray[i+12][1] + dataArray[i+24][1] + dataArray[i+36][1] + dataArray[i+48][1])/5;
			predictionArray[i][2] = (dataArray[i][2] + dataArray[i+12][2] + dataArray[i+24][2] + dataArray[i+36][2] + dataArray[i+48][2])/5;
		}
		
		double[][] newDataArray = append(dataArray, predictionArray);
		
		for(int j=12; j < 24 ; j++){
			predictionArray[j][0] = (newDataArray[j][0] + newDataArray[j+12][0] + newDataArray[j+24][0] + newDataArray[j+36][0] + newDataArray[j+48][0])/5;
			predictionArray[j][1] = (newDataArray[j][1] + newDataArray[j+12][1] + newDataArray[j+24][1] + newDataArray[j+36][1] + newDataArray[j+48][1])/5;
			predictionArray[j][2] = (newDataArray[j][2] + newDataArray[j+12][2] + newDataArray[j+24][2] + newDataArray[j+36][2] + newDataArray[j+48][2])/5;
		}
	 ...
	 * @param a A 2D double array at needs another 2D double array attached.
	 * @param b A 2D double array that needs to be attached to the end of another 2D double array.
	 * @return The 2D double array created when the second parameter is appended to the first.
	 */
	  ...
	  * @param array The 2D double array to be printed to console.
	  */
