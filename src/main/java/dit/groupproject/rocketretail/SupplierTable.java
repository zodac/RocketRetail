...
public class SupplierTable {
	...
	static boolean reverse = false;
	 //Methods 
    /** 
     * Creates the JMenuItem for "Supplier", and defines the ActionListener for the JMenuItem. <br /> 
     ...
     * Calls {@link #showSupplierInfo(Supplier)} when a supplier is selected from the table. 
     *   *  
     * @see     #add() 
     * @see     #edit(int) 
	...
	public static void supplier(){
		...
		for(int i = 0; i < ShopDriver.suppliers.size()+1; i++){
			if(i < ShopDriver.suppliers.size())
				supplierMemberArrayEdit[i+1] = "ID: " + doubleFormatter.format(ShopDriver.suppliers.get(i).getSupplierID()) + " (" + ShopDriver.suppliers.get(i).getSupplierName() + ")";
		}
		...
		for(int i = 0; i < ShopDriver.suppliers.size()+1; i++){
			if(i < ShopDriver.suppliers.size())
				supplierMemberArrayDelete[i+1] = "ID: " + doubleFormatter.format(ShopDriver.suppliers.get(i).getSupplierID()) + " (" + ShopDriver.suppliers.get(i).getSupplierName() + ")";
		}
	 ...
	 /** 
     * Opens a form to input information for a new Supplier in the left Panel of the screen. Uses GridBagLayout.<br /> 
     * Adds JButtons to cancel, or save new supplier and re-runs {@link #supplier()} 
     *  
     * @see #supplier() 
     */
	public static void add(){
        ...
        save.addActionListener(new ActionListener(){  
            ...
            public void actionPerformed(ActionEvent e){
            	...
            	if(valid){
					ShopDriver.suppliers.add(new Supplier(Integer.parseInt(suppIDField.getText()),   
														  suppNameField.getText(),
     ...
     * Adds JButtons to cancel, or save supplier changes and re-runs {@link #supplier()} 
     *  
     * @param supplierID     an Integer specifying the supplier to edit 
     *  
     * @see                 #supplier() 
     */
	public static void edit(int supplierID){
		...
		for(Supplier s : ShopDriver.suppliers){
   			if (supplierID == s.getSupplierID()){
                ...
                lastPurchaseYear.setSelectedIndex(Integer.parseInt(s.getLastPurchase().substring(6, 10))-(ShopDriver.yearStart-1));
                ...
                dateAddedYear.setSelectedIndex(Integer.parseInt(s.getDateAdded().substring(6, 10))-(ShopDriver.yearStart-1));
                ...
                save.addActionListener(new ActionListener(){  
                    public void actionPerformed(ActionEvent e){
                    	...
                    	if(valid){
							ShopDriver.suppliers.add(index, new Supplier(Integer.parseInt(suppIDField.getText()),
																		 suppNameField.getText(),
																		 phoneNoField.getText(),
																		 addressField.getText(),
																		 vatNoField.getText(),   
	 ...
	 /** 
     * Opens a confirmation panel asking the user to confirm whether to delete the selected supplier or not.<br /> 
     * Both parameters are passed using the selected option in the deletion JComboBox.<br /> 
     * Re-runs {@link #supplier()} on completion. 
     *  
     * @param supplierID     an Integer specifying the ID number of the supplier chosen to be deleted 
     * @param supplierName   a String specifying the name/description of the supplier chosen to be deleted 
     *  
     * @see                 #supplier() 
     */
	public static void delete(int supplierID, String supplierName){
	    ...
	    if (ShopDriver.showDialog("Please confirm", myPanel) == JOptionPane.OK_OPTION){
	    	...
	    	for(Supplier c : ShopDriver.suppliers){
	    		if(supplierID == c.getSupplierID())
	    			i = ShopDriver.suppliers.indexOf(c); //Object can't be removed while being accessed - save its index location for now
		    }
	    }
	 ...
	 /** 
     * Creates a new page on-screen with a detailed breakdown of purchases from an individual supplier.<br /> 
     * Displays basic information (description, Product ID, stock level, current stock) 
     * just as the table, but also includes a JTable showing all-time purchases.<br />  
     ...
     *  
     * @param s     the Supplier whose information is displayed on-screen 
     *  
	...
	public static void showSupplierInfo(Supplier s){
		...
		JTextField supplierField = new JTextField(s.getSupplierName() + " (" + doubleFormatter.format(s.getSupplierID()) + ")", textFieldSize);
		...
		for(int i = 0; i < ShopDriver.products.size(); i++){
			...
			if(s.getSupplierID() == ShopDriver.products.get(i).getSupplierID()){
				...
				data[indexArray][3] = ShopDriver.products.get(i).getStockLevel() + "/" + ShopDriver.products.get(i).getMaxLevel();
				indexArray++;
			}
	...
	/** 
     * Calculates the first available ID number to use when creating a new supplier in {@link #add()}. 
     *  
	...
	public static int firstAvailableID(){
		...
		if(found)
			return output;
		else return ShopDriver.suppliers.size()+ShopDriver.supplierIDStart;
	}
	 /** 
	...
	/** 
     * Sorts {@link ShopDriver#suppliers} by supplier name in alphabetical order. 
     *  
	...
	/** 
     * Sorts {@link ShopDriver#suppliers} by supplier address in alphabetical order. 
     *  
     * @param reverse       a boolean which specifies whether to sort in reverse order or not 
     */
	...
	/** 
     * Sorts {@link ShopDriver#suppliers} by supplier VAT Number in alphabetical order. 
     *  
     * @param reverse       a boolean which specifies whether to sort in reverse order or not 
     */
