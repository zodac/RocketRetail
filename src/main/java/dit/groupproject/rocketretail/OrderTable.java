...
public class OrderTable {
    ...
    public static void order() {
        ...
        if (filter.equals("Show All Orders")) {
            ...
            for (int i = 0; i < ShopDriver.orders.size(); i++) {
                ...
                if (ShopDriver.orders.get(i).isSupplier())
                    ...
                    totalPrice = ShopDriver.orders.get(i).getTotalSale();

                if (ShopDriver.orders.get(i).getDeliveryDate().length() == 0)
                    delivery = " ";
                else
                    delivery = ShopDriver.orders.get(i).getDeliveryDate();
            ...
            for (int i = 0; i < ShopDriver.orders.size() + 1; i++) {
                if (i < ShopDriver.orders.size() && ShopDriver.orders.get(i).isActive()) {
                    orderArrayComplete[arrayIndex] = "ID: "
                            + doubleFormatter.format(ShopDriver.orders.get(i).getOrderID());
                    arrayIndex++;
                }
            }
            ...
            for (int i = 0; i < ShopDriver.orders.size() + 1; i++) {
                if (i < ShopDriver.orders.size() && ShopDriver.orders.get(i).isActive()
                        && ShopDriver.orders.get(i).isSupplier()) {
                    orderArrayComplete[arrayIndex] = "ID: "
                            + doubleFormatter.format(ShopDriver.orders.get(i).getOrderID());
                    arrayIndex++;
                }
        ...
        String[] filterOptions = { "Show All Orders", "Show Supplier Orders", "Show Customer Orders",
                "Show Active Orders" };
        ...
        customerOptions.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (customerOptions.getSelectedItem().equals("New Customer"))
                    ...
                    // Make font bold
                    productName.setFont(new Font(productName.getFont().getFontName(), Font.BOLD, productName.getFont()
                            .getSize()));
                    currentStockLevel.setFont(new Font(productName.getFont().getFontName(), Font.BOLD, productName
                            .getFont().getSize()));
                    orderAmount.setFont(new Font(productName.getFont().getFontName(), Font.BOLD, productName.getFont()
                            .getSize()));
                        ...
                        public void actionPerformed(ActionEvent e) {
                            ...
                            for (JLabel label : productLabels) {
                                if (orderAmountFields.get(productLabels.indexOf(label)).getText().length() == 0
                                        || Integer.parseInt(orderAmountFields.get(productLabels.indexOf(label))
                                                .getText()) > ShopDriver.products.get(productLabels.indexOf(label))
                                                .getStockLevel()) {
                                    valid = false;
                                    ...
                                    count += Integer.parseInt(orderAmountFields.get(productLabels.indexOf(label))
                                            .getText());
                                }
                            }
                            ...
                            if (valid) {
                                ...
                                if (items.size() > 0) {
                                    ...
                                    if (items.size() > 0)
                                        ShopDriver.orders.add(new Order(ShopDriver.orders.size(),
                                                ShopDriver.currentStaff.getStaffID(), traderID, new SimpleDateFormat(
                                                        "dd/MM/yyyy").format(new Date()), items, true));
                                ...
                                ShopDriver.setConfirmMessage("Order #"
                                        + doubleFormatter.format(ShopDriver.orders.get(ShopDriver.orders.size() - 1)
                                                .getOrderID()) + " created for customer \""
                                        + activeCust.getCustomerName() + "\"");
                        ...
                        public void actionPerformed(ActionEvent e) {
                            ...
                            for (JLabel label : productLabels) {
                                if (orderAmountFields.get(productLabels.indexOf(label)).getText().length() == 0
                                        || (Integer.parseInt(orderAmountFields.get(productLabels.indexOf(label))
                                                .getText()) + ShopDriver.products.get(productLabels.indexOf(label))
                                                .getStockLevel()) > ShopDriver.products.get(
                                                productLabels.indexOf(label)).getMaxLevel()) {
                                    valid = false;
                                    orderAmountFields.get(productLabels.indexOf(label)).setBorder(
                                            BorderFactory.createMatteBorder(1, 1, 1, 1, Color.red));
                                } else {
                            ...
                            if (valid) {
                                ...
                                if (items.size() > 0) {
                                    ShopDriver.orders.add(new Order(ShopDriver.orders.size(), ShopDriver.currentStaff
                                ...
                                ShopDriver.setConfirmMessage("Order #"
                                        + doubleFormatter.format(ShopDriver.orders.get(ShopDriver.orders.size() - 1)
                                                .getOrderID()) + " created for supplier \""
                                        + activeSupp.getSupplierName() + "\"");
    ...
    public static void showOrderInfo(Order o) {
        ...
        orderDateLabel.setFont(new Font(orderDateLabel.getFont().getFontName(), Font.BOLD, orderDateLabel.getFont()
                .getSize()));
        deliveryDateLabel.setFont(new Font(deliveryDateLabel.getFont().getFontName(), Font.BOLD, deliveryDateLabel
                .getFont().getSize()));
