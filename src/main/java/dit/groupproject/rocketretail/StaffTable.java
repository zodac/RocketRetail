...
public class StaffTable {
        ...
        for (int i = 0; i < ShopDriver.staffMembers.size() + 1; i++) {
            if (i < ShopDriver.staffMembers.size())
                staffMemberArrayEdit[i + 1] = "ID: "
                        + doubleFormatter.format(ShopDriver.staffMembers.get(i).getStaffID()) + " ("
                        + ShopDriver.staffMembers.get(i).getStaffName() + ")";
        }
    ...
    public static void add() {
        ...
        save.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (valid) {
                    ShopDriver.staffMembers.add(new Staff(Integer.parseInt(idField.getText()), Integer.parseInt(String
                            .valueOf(pinField.getPassword())), nameField.getText(), genderField.getSelectedIndex(),
                            phoneNoField.getText(), addressField.getText(), Double.parseDouble(wageField.getText()),
            ...
            if (ShopDriver.showDialog("Please confirm", myPanel) == JOptionPane.OK_OPTION) {
                ...
                for (Staff s : ShopDriver.staffMembers) {
                    if (staffID == s.getStaffID())
                        i = ShopDriver.staffMembers.indexOf(s); // Object can't
                                                                // be removed
                                                                // while being
                                                                // accessed -
                                                                // save its
    ...
    public static int firstAvailableID() {
        ...
        if (found)
            ...
            return ShopDriver.staffMembers.size();
    }
